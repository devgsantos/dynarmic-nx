...
void A32AddressSpace::RegisterNewBasicBlock(IR::LocationDescriptor location,
                                            IR::LocationDescriptor end_location,
                                            const EmittedBlockInfo& block_info) {
    const A32::LocationDescriptor descriptor{location};
    const A32::LocationDescriptor typed_end_location{end_location};
    const auto range = boost::icl::discrete_interval<u32>::closed(
        descriptor.PC(), typed_end_location.PC() - 1);
    ...
    std::snprintf(buffer.data(), buffer.size(),
                  ...
                  descriptor.PC(), typed_end_location.PC(),
