...
public:
    ...
    HaltReason Run(A32AddressSpace& process, A32JitState& thread_ctx, volatile u32* halt_reason) {
        ...
        assert(process.HostGetTicksRemainingTarget() != 0);
        assert(process.HostAddTicksTarget() != 0);
        assert(azahar_switch_dynarmic_jit_get_range_id(
                   static_cast<std::uintptr_t>(process.HostGetTicksRemainingTarget())) == 0);
        assert(azahar_switch_dynarmic_jit_get_range_id(
                   static_cast<std::uintptr_t>(process.HostAddTicksTarget())) == 0);
